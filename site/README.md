...
| `/libros/` | `pages/libros.astro` | Book listing (no downloads — private) |
...
**Current output:** 1,887 static pages (1,870 articles + 10 events + 7 other pages).

## Data sources

### `src/data/catalog.json` (generated)

Main content database. Generated by `../build_catalog.py` from Docling-converted documents. Each entry has:
...
- **Header** — Site name, 7-item navigation bar, active link highlighting
...
- Brand colors: `brand-900` (#1a2744, dark navy) through `brand-50` (#f4f6fb, light)
- Font stacks: Inter/system sans for UI, Georgia/serif for article body
...
Article detail pages show a "Descargar documento original (IPFS)" link when available.
...
│   ├── styles/
│   │   └── global.css         # Tailwind + brand theme + article typography
│   ├── data/
│   │   ├── catalog.json       # Generated content database (1,944 items)
│   │   └── libros.json        # Manual book list (11 books)
│   └── pages/
│       ├── index.astro
│       ├── articulos/
│       │   ├── index.astro    # List + filters
│       │   └── [slug].astro   # Detail + prev/next
│       ├── hechos/
│       │   ├── index.astro    # Event list
│       │   └── [event].astro  # Event docs (collapsible)
│       ├── libros.astro
│       ├── audios.astro
│       ├── videos.astro
